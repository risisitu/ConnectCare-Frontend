  ...
  useEffect(() => {
    const fetchPatientsForDoctor = async () => {
      ...
      try {
        ...
        const mapped: PatientRow[] = patients.map((pt: any, idx: number) => {
          ...
          const image = pt.profile_image ?? pt.profileImage ?? pt.image ?? "./images/user/user-17.jpg";
          const contact = pt.phone_number ?? pt.phone ?? pt.contact ?? "-";
          const gender = pt.gender ?? "-";
          const condition = pt.condition ?? "-";
                    ...
                    <TableCell className="px-5 py-4 sm:px-6 text-start">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 overflow-hidden rounded-full">
                          <img width={40} height={40} src={p.image} alt={p.name} />
                        </div>
                        <div>
