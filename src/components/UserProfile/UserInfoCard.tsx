...
export default function UserInfoCard() {
  ...
  const isPatient = "blood_group" in profile;
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            {isPatient && (
              <>
                <div>
                  <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Blood Group</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{(profile as any).blood_group}</p>
                </div>

                <div>
                  <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Gender</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{(profile as any).gender}</p>
                </div>

                <div className="lg:col-span-2">
                  <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Date of Birth</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{new Date((profile as any).date_of_birth).toLocaleDateString()}</p>
                </div>

                <div className="lg:col-span-2">
                  <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Allergies</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{(profile as any).allergies}</p>
                </div>
              </>
            )}
          </div>
                  ...
                  {isPatient && (
                    <>
                      <div className="col-span-2 lg:col-span-1">
                        ...
                        <Input type="text" value={bloodGroup} onChange={(e) => setBloodGroup(e.target.value)} disabled />
                      </div>
                      <div className="col-span-2 lg:col-span-1">
                        <Label>Gender</Label>
                        <Input type="text" value={gender} onChange={(e) => setGender(e.target.value)} disabled />
                      </div>
                      <div className="col-span-2 lg:col-span-1">
