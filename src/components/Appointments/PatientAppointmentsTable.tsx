...
export default function PatientAppointmentsTable() {
        ...
        const fetchAppointments = async () => {
            try {
                ...
                const response = await fetch(
                    `${import.meta.env.VITE_API_URL}/api/patients/appointments`,
                    {
                        headers: {
    ...
    const initiateCall = (app: Appointment) => {
        // Target is doctor
        const targetId = app.doctor_id;
        const targetName = `Dr. ${app.doctor_first_name} ${app.doctor_last_name}`;
    ...
    return (
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-4 pb-3 pt-4 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6">
                                    ...
                                    <TableCell className="py-3">
                                        <div className="flex flex-col">
                                            ...
                                            {app.doctor_specialization && (
                                                <span className="text-xs text-gray-400">{app.doctor_specialization}</span>
                                            )}
                                        </div>
                                    </TableCell>
            ...
            <VideoCallModal
                ...
                localUser={user ? { id: user.id || "unknown", name: user.name || user.email || "Patient" } : { id: "pat", name: "Patient" }}
